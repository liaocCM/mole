# Trendshift 月度報告 — 2026-02-16 至 2026-03-23
**涵蓋範圍：** 5 週（35 天）  |  **不重複專案（2+ 週上榜）：** 83
...
**Language:** N/A  |  **Stars:** 132,899  |  **Forks:** 33,591  |  **Best Score:** 24,578  |  **Best Rank:** #1  |  **Weeks on chart:** 3/5  |  **Days on chart:** 8/35  |  **Created:** 2025-03-05
...
**背景：** autoresearch 是 Andrej Karpathy 於 2026 年 3 月發布的自主 AI 研究框架，讓 AI 代理在單一 GPU 上自動進行 LLM 訓練實驗。代理修改程式碼、訓練 5 分鐘、檢查結果是否改善，保留或丟棄後重複循環。基於 nanochat 的簡化單 GPU 實作，研究人員不再直接修改 Python 程式碼，而是編寫 Markdown 格式的 program.md 來配置自主研究組織。已累積超過 50,000 顆星。
**解決的問題：** 傳統 AI 研究需要研究人員手動設計實驗、訓練模型、分析結果並迭代——這個循環既耗時又受限於人類的工作時間。autoresearch 讓代理在夜間自主運行完整的實驗循環，研究人員早上醒來即可查看實驗日誌和（希望是）更好的模型。
...
**背景：** gstack 是 Y Combinator 總裁兼 CEO Garry Tan 開發的 Claude Code 工具套件，包含 15 個專業工具，將 Claude Code 轉變為虛擬工程團隊。Tan 聲稱在過去 60 天內產出超過 60 萬行生產程式碼（35% 測試），每天 10,000-20,000 行，同時全職運營 YC。已超過 39,000 顆星。
**解決的問題：** 單人開發者使用 AI 編碼助手時缺乏系統化的品質控制流程——沒有設計審查、架構鎖定、發布管理或 QA 驗證。gstack 透過模擬完整工程團隊的角色分工，為獨立開發者提供企業級的開發流程。
**為何又一個？** Garry Tan 的 YC 背景和 Palantir、Posterous 的創業經歷賦予專案獨特的可信度。「同一個人，不同時代」的敘事——2013 年 772 次貢獻對比 2026 年 1,237 次貢獻——有力地展示了 AI 工具對個人生產力的倍增效應。
...
**背景：** Project N.O.M.A.D.（Node for Offline Media, Archives, and Data）是 Crosstalk Solutions 開發的離線優先知識和教育伺服器，可安裝在任何基於 Debian 的系統上。它將維基百科、醫療指南、地圖和 AI 模型打包到單一可部署的伺服器中，透過瀏覽器存取。已超過 14,000 顆星。
**解決的問題：** 在網路中斷、自然災害或偏遠地區等場景中，無法存取線上知識和工具可能危及生命。Project N.O.M.A.D. 將關鍵知識和 AI 能力離線化，確保在任何情況下都能獲取重要資訊。
**為何又一個？** 完全離線運行、Docker Compose 一鍵部署和內建 AI 的組合使其有別於簡單的離線維基百科鏡像。其生存導向的設計理念——而非僅是便利性——瞄準了準備主義者和偏遠地區工作者的獨特需求。
...
**Language:** N/A  |  **Stars:** 27,035  |  **Forks:** 2,259  |  **Best Score:** 13,160  |  **Best Rank:** #1  |  **Weeks on chart:** 4/5  |  **Days on chart:** 18/35  |  **Created:** 2026-02-08
...
**Language:** N/A  |  **Stars:** 77,406  |  **Forks:** 10,497  |  **Best Score:** 11,544  |  **Best Rank:** #1  |  **Weeks on chart:** 2/5  |  **Days on chart:** 4/35  |  **Created:** 2016-10-21
...
**背景：** Context Hub 由 Andrew Ng 發布，是一個為編碼代理提供策展、版本化文件的 CLI 工具。代理常常虛構 API 並遺忘會話中學到的知識，Context Hub 讓它們獲取精確的文件，並透過註解機制在會話間累積學習。所有內容以 Markdown 維護在開源倉庫中。已超過 11,000 顆星。
**解決的問題：** 編碼代理經常虛構 API 呼叫方式或使用過時的文件，導致生成的程式碼無法運行。Context Hub 提供經策展的、版本化的、語言特定的 API 文件，確保代理始終使用正確且最新的參考資料。
**為何又一個？** Andrew Ng 的 AI 教育領域影響力和開放的內容模式——所有文件都是可檢視的 Markdown——使其有別於封閉的知識庫方案。代理可自主註解和回饋的機制使文件品質隨使用而持續改善。
...
**背景：** Deep Agents 是 LangChain 推出的「電池內含」代理套件，提供開箱即用的完整代理——規劃、檔案系統操作、Shell 存取、子代理委派和智慧上下文管理。開發者無需自行組裝提示、工具和上下文管理，直接獲得可運行的代理並按需自訂。已超過 17,000 顆星。
**解決的問題：** 建構功能完整的代理需要組裝規劃、檔案操作、命令執行和上下文管理等多個組件，這些整合工作既耗時又容易出錯。Deep Agents 將這些標準化為內建功能，開發者只需自訂業務邏輯。
**為何又一個？** LangChain 在代理框架領域的主導地位和龐大的使用者基礎使 Deep Agents 能快速獲得採用。其「代理套件」定位——而非底層框架——使其與 LangChain 自身的低階組件形成互補而非競爭。
...
**Language:** N/A  |  **Stars:** 281,590  |  **Forks:** 12,949  |  **Best Score:** 7,228  |  **Best Rank:** #6  |  **Weeks on chart:** 2/5  |  **Days on chart:** 3/35  |  **Created:** 2015-06-01
...
**Language:** N/A  |  **Stars:** 41,386  |  **Forks:** 3,957  |  **Best Score:** 7,159  |  **Best Rank:** #3  |  **Weeks on chart:** 4/5  |  **Days on chart:** 24/35  |  **Created:** 2026-01-25
...
**背景：** claude-mem 由 thedotmack 開發，是一個 Claude Code 插件，能自動記錄編碼會話中 Claude 的所有操作，使用 Anthropic Agent SDK 進行 AI 壓縮，並在未來會話中自動注入相關上下文。支援 22 種以上語言的文件。已累積近 40,000 顆星。
**解決的問題：** Claude Code 的會話之間沒有持久記憶——每次新會話都從零開始，開發者需要反覆解釋專案上下文和偏好。claude-mem 自動擷取和壓縮會話知識，使 Claude 在新會話中能延續先前的學習。
**為何又一個？** 自動擷取而非手動記錄的方式大幅降低了使用門檻。基於 Anthropic Agent SDK 的 AI 壓縮確保記憶條目簡潔且相關，避免了上下文視窗的浪費。其近 40,000 的星數反映了跨會話記憶問題的普遍性。
...
**Language:** N/A  |  **Stars:** 5,850  |  **Forks:** 553  |  **Best Score:** 5,826  |  **Best Rank:** #7  |  **Weeks on chart:** 2/5  |  **Days on chart:** 4/35  |  **Created:** 2025-05-19

**背景：** Marketing for Founders 由 Edo Stradella 編寫，是一個針對技術創辦人的實用行銷資源集合，專注於零預算情況下獲取早期使用者。涵蓋從發布平台、社群媒體、SEO、冷外展到內容行銷等 18 個以上主題。已超過 5,800 顆星。
**解決的問題：** 大多數行銷建議針對的是有大預算的 VC 投資新創公司如何擴展到百萬 ARR，對零預算的早期技術創辦人毫無幫助。這個集合專注於獲取前 10-1000 位使用者的實際可行策略。
**為何又一個？** 針對早期階段（前 1000 位使用者）和零預算場景的精確定位使其有別於泛泛的行銷建議集合。涵蓋 LLM SEO/AEO/GEO 等新興行銷渠道的內容反映了 2026 年的行銷現實。
...
**背景：** OpenDataLoader PDF 是一個為 AI 管道設計的 PDF 解析器，能將 PDF 轉換為 Markdown、JSON、HTML 和 Tagged PDF。支援 Python、Node.js 和 Java SDK，在混合模式下整體擷取準確率排名第一（0.90），表格擷取準確率達 0.93。與 PDF Association 合作，遵循 Well-Tagged PDF 規範。已超過 9,000 顆星。
**解決的問題：** 為 RAG 和 LLM 管道準備 PDF 資料時，現有解析器在複雜佈局、表格和掃描文件上表現不佳，且缺乏邊界框資訊。OpenDataLoader PDF 提供確定性輸出、每個元素的邊界框和 XY-Cut++ 閱讀順序演算法，確保高品質的結構化資料擷取。
**為何又一個？** 基準測試第一的擷取準確率、業界首個開源端到端 PDF 自動標記功能，以及與 PDF Association 的官方合作，使其在 PDF 解析器中獨樹一幟。多語言 SDK 支援降低了不同技術棧的整合門檻。
...
**Language:** N/A  |  **Stars:** 5,385  |  **Forks:** 497  |  **Best Score:** 5,010  |  **Best Rank:** #13  |  **Weeks on chart:** 2/5  |  **Days on chart:** 4/35  |  **Created:** 2026-02-19

**背景：** Taste-Skill 由 Leonxlnx 開發，是一組提升 AI 工具前端程式碼品質的技能集合。它教導 AI 建構具備適當動畫、間距和視覺品質的現代高端設計，而非生成通用無聊的介面。相容 Cursor、Claude Code、Antigravity 等多個代理平台。已超過 5,300 顆星。
**解決的問題：** AI 編碼工具生成的前端介面往往千篇一律——相同的紫色漸層、相同的佈局模式、相同的動畫（或缺乏動畫）。Taste-Skill 透過注入設計智慧，使 AI 輸出具備專業設計師水準的視覺品質。
**為何又一個？** 專注於「品味」而非功能的獨特定位使其有別於通用的前端技能。包含多種風格變體（高端、極簡、柔和）和「反 AI 垃圾」設計原則，直接針對 AI 生成內容的最大痛點。
...
**背景：** UI UX Pro Max 是由 nextlevelbuilder 開發的 AI 技能，包含 161 條推理規則和 67 種 UI 風格，為 AI 代理提供跨平台的專業 UI/UX 設計智慧。支援 Claude Code、Cursor、Codex 等多個代理平台，並提供 CLI 工具和 Python SDK。已累積超過 49,000 顆星。
**解決的問題：** AI 代理在生成 UI 時缺乏系統化的設計原則指導，導致輸出的介面在視覺一致性、可用性和專業度方面參差不齊。UI UX Pro Max 透過 161 條推理規則為代理提供結構化的設計決策框架。
**為何又一個？** 161 條推理規則和 67 種 UI 風格的廣度遠超同類技能。其近 49,000 的星數表明開發者對系統化 UI/UX 設計智慧的強烈需求——不只是範本，而是可推理的設計原則。
...
**背景：** Lightpanda 是一個從零開始用 Zig 語言建構的無頭瀏覽器，專為 AI 代理和自動化設計——不是 Chromium 分支，也不是 WebKit 補丁。它比 Chrome 快 11 倍、記憶體佔用少 9 倍，支援透過 CDP 相容 Playwright、Puppeteer 和 chromedp。已超過 24,000 顆星。
**解決的問題：** 現有的無頭瀏覽器（如 headless Chrome）是為通用網頁瀏覽設計的，在 AI 代理和大規模自動化場景中記憶體消耗過高、啟動速度慢。Lightpanda 從底層為這些使用場景最佳化，實現了數量級的效能提升。
**為何又一個？** 從零建構而非基於 Chromium 分支的方法使其能實現根本性的效能改進。Zig 語言帶來的低階控制和記憶體安全，以及與現有自動化生態系統（Playwright、Puppeteer）的 CDP 相容性，使其能無縫替換 Chrome 而無需重寫測試程式碼。
...
**背景：** Get Shit Done（GSD）是由 TACHES 開發的 Claude Code 元提示和上下文工程系統，解決「上下文腐爛」問題——當 Claude 填滿上下文視窗時品質開始下降。支援 Claude Code、OpenCode、Gemini CLI、Codex、Copilot 和 Antigravity。已超過 39,000 顆星。
**解決的問題：** 隨著對話長度增加，AI 代理的輸出品質會因上下文視窗填滿而逐漸下降。GSD 透過規格驅動的開發方法和元提示工程，確保代理在整個會話中維持一致的高品質輸出。
**為何又一個？** 直接針對「上下文腐爛」這一被廣泛體驗但少有工具專門解決的問題。跨平台相容性（6 個代理平台）和 npm 一鍵安裝的便利性使其成為解決此問題的首選工具。
...
**背景：** Frontend Slides 由 zarazhangrui 開發，是一個 Claude Code 技能，讓非設計師能建立精美的動畫豐富的 HTML 簡報——從零開始或透過轉換 PowerPoint 檔案。採用「展示而非描述」的方法，生成視覺預覽讓使用者選擇喜好的風格。輸出為零依賴的單一 HTML 檔案。已超過 11,000 顆星。
**解決的問題：** 建立專業簡報通常需要設計技能或昂貴的工具。Frontend Slides 讓不會 CSS 或 JavaScript 的使用者也能透過自然語言對話產出具備生產品質的網頁簡報。
**為何又一個？** 零依賴的單一 HTML 輸出、PPT 轉換功能和「反 AI 垃圾」風格策展的組合，使其有別於通用的 AI 簡報工具。視覺風格發現機制——讓使用者從預覽中選擇而非用文字描述——是獨特的互動設計。
...
**背景：** Kitten TTS 是 KittenML 推出的開源輕量文字轉語音函式庫，基於 ONNX 建構。模型參數從 15M 到 80M（25-80 MB），能在 CPU 上高品質合成語音而不需要 GPU。提供 8 種內建語音和商業支援選項。已超過 13,000 顆星。
**解決的問題：** 現有的高品質 TTS 模型通常體積龐大且需要 GPU 運行，使其難以部署在邊緣裝置和資源受限的環境中。Kitten TTS 在 25MB 的極小體積下實現了最先進的語音品質，純 CPU 推理即可運行。
**為何又一個？** 25MB 模型大小與最先進品質的結合是對 TTS 領域的重大突破。ONNX 格式確保了跨平台部署的便利性，而 Apache 2.0 授權和商業支援選項使其適用於從開源專案到企業部署的各種場景。
...
**背景：** Open Alice 是一個檔案驅動的 AI 交易代理，將研究部門、量化團隊、交易大廳和風險管理整合到筆電上全天候運行。Markdown 定義人設和任務、JSON 定義配置、JSONL 儲存對話——人類和 AI 都透過讀寫檔案來控制 Alice。支援 Claude、OpenAI 等多個 AI 供應商。已超過 2,800 顆星。
**解決的問題：** 個人交易者缺乏機構級的完整交易基礎設施——研究、量化分析、執行和風險管理通常需要整個團隊。Open Alice 將這些角色自動化為單一 AI 代理，在個人筆電上即可運行。
**為何又一個？** 「Vibe Coding 到 Vibe Trading」的理念——將驅動 AI 編碼的相同檔案讀寫原語直接應用於交易——是獨特的設計哲學。無資料庫、無容器、純檔案的架構使其對開發者極為透明和可審計。
...
**Language:** N/A  |  **Stars:** 448,089  |  **Forks:** 33,704  |  **Best Score:** 3,847  |  **Best Rank:** #6  |  **Weeks on chart:** 2/5  |  **Days on chart:** 3/35  |  **Created:** 2014-07-11
...
> GEO 優先的 SEO 技能，適用於 Claude Code。全面的 AI 搜尋最佳化——可引用性評分、AI 爬蟲分析、品牌權威度、Schema 標記和 PDF 報告
...
**背景：** GEO-SEO Claude 由 Zubair Trabzada 開發，是一個 Claude Code 技能，專注於為 AI 驅動的搜尋引擎（ChatGPT、Claude、Perplexity、Gemini、Google AI Overviews）最佳化網站，同時維持傳統 SEO 基礎。提供可引用性評分、AI 爬蟲存取分析、品牌提及掃描等功能。已超過 3,600 顆星。
**解決的問題：** AI 搜尋正在蠶食傳統搜尋流量（年增長 527%），但只有 23% 的行銷人員在投資 GEO（生成式引擎最佳化）。這個技能為網站提供全面的 AI 搜尋可見度審計和最佳化建議。
**為何又一個？** 「GEO 優先，SEO 輔助」的定位精準捕捉了 2026 年搜尋流量遷移的趨勢。整合到 Claude Code 的技能格式使其可以一條命令完成完整審計，而非需要另外使用的獨立工具。
...
**Language:** N/A  |  **Stars:** 10,588  |  **Forks:** 867  |  **Best Score:** 2,874  |  **Best Rank:** #7  |  **Weeks on chart:** 3/5  |  **Days on chart:** 3/35  |  **Created:** 2026-01-18
...
**背景：** ClawRouter 由 BlockRunAI 開發，是唯一讓 AI 代理能獨立運作的 LLM 路由器——代理無法註冊帳戶或輸入信用卡，但可以簽署交易。它支援 41 個以上模型、亞毫秒本機路由，並透過 x402 協定實現 USDC 微支付。曾獲 USDC 黑客松代理商務獎。已超過 6,300 顆星。
**解決的問題：** 自主 AI 代理需要存取多個 LLM 供應商，但傳統的 API 密鑰模式要求人類註冊帳戶和管理付款。ClawRouter 透過區塊鏈原生的微支付機制讓代理能自主選擇和付費使用 LLM 服務。
**為何又一個？** 「代理原生」設計理念——零 API 密鑰、以加密貨幣支付——瞄準了代理自主性的關鍵瓶頸：經濟獨立。USDC 黑客松獲獎證明了其在代理商務領域的創新價值。
...
**Language:** N/A  |  **Stars:** 42,352  |  **Forks:** 5,387  |  **Best Score:** 2,270  |  **Best Rank:** #19  |  **Weeks on chart:** 2/5  |  **Days on chart:** 2/35  |  **Created:** 2022-08-15
...
> **本月主題：** 自主 AI 研究與「一人軍團」開發模式雙線並進——Karpathy 的 autoresearch 讓代理自主運行 ML 實驗、Garry Tan 的 gstack 展示單人日產兩萬行程式碼的可能；代理基礎設施持續成熟，從 Lightpanda 的從零建構瀏覽器到 ClawRouter 的代理原生支付；OpenClaw 和 Claude Code 生態系統全面開花，設計技能（Taste-Skill、UI UX Pro Max）、記憶系統（claude-mem）和上下文工程（GSD、Context Hub）填補了代理工作流程的最後幾塊拼圖。
